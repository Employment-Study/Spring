...
// 스프링4 에서는 extends HandlerInterceptorAdapter
// 스프링5 에서는 implements HandlerInterceptor
...
public class LoginInterceptor implements HandlerInterceptor {
	...
	@Override	// preHandle : 요청이 컨트롤러에 도달하기 '전에' 작동하는 함수
				// 아직 컨트롤러에 요청이 도달하기 전이므로, 이후 작업을 계속할지말지에 대해 반환값을 전달할 수 있다
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if(login == null) {
			...
			return false;	// 원래 예정되어있던 컨트롤러의 함수로 진입하지 않고, 새로 만들어준 응답을 활용한다
							// (continue: false)
		}
		
		return true;	// 원래 예정되어있던 컨트롤러의 함수로 그대로 연결한다(continue: true)
	}
	...
	@Override	// afterCompletion : jsp로 포워딩된후 응답이 서버에서 클라이언트에게 출발한 이후 작동하는 함수
